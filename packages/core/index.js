...
function scanPrompt(text, options) {
  ...
  var socialEng = detect.hasSocialEngineering(input);
  var exfiltrate = detect.hasExfiltrationMatch(matches);
  // Skip dampening when social engineering authority framing is present,
  // even if text also contains benign markers (e.g., "documentation")
  var dampened = benign && !socialEng;
  var score = detect.applyDampening(rawScore, dampened, exfiltrate);
...
module.exports = {
  ...
  hasSocialEngineering: detect.hasSocialEngineering,
