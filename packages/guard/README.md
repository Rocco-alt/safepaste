...
Agent runtime security middleware — scans tool inputs and outputs for prompt injection using [@safepaste/core](https://www.npmjs.com/package/@safepaste/core).

## Why

AI agents call external tools (web search, file read, code execution) whose outputs can contain **indirect prompt injection** payloads. Guard wraps your tool functions to scan text flowing through the agent pipeline — before a tool executes (input) and after it returns (output).
