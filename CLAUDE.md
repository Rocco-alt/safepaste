...
SafePaste is a prompt-injection detection tool with three delivery mechanisms: a Chrome extension (client-side, fully local), a REST API (server-side), and agent runtime middleware. It's a monorepo with six packages under `packages/`.
...
npm test                 # Run all tests: core (194) + test (88) + guard (128) + API (37)
...
- **packages/guard/** — Agent runtime security middleware (`@safepaste/guard`). Wraps tool functions to scan inputs/outputs for prompt injection. `createGuard()` factory with 4 modes (log/warn/block/callback), per-direction mode config, fail-open scanning. Works with OpenAI SDK, Vercel AI SDK, LangChain, custom agent loops. Uses `@safepaste/core` as peer dependency.
