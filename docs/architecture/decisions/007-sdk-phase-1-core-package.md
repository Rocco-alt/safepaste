...
## Extensibility Note (2026-03-18)

The `scanPrompt()` response shape (`{ flagged, risk, score, threshold, matches[], meta{} }`) is the stable public API surface. When automated intelligence capabilities are added (ADR-010), the response may gain optional fields — for example, `meta.intelligence: { mlScore, confidence, signals[] }` — in a future minor version. This would be additive and non-breaking: existing consumers that don't read intelligence fields continue to work unchanged. The `flagged` and `score` fields will always reflect deterministic enforcement; intelligence signals will be informational only unless the consumer explicitly opts in to intelligence-aware scoring.
