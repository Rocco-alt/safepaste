...
## Scope Clarification (2026-03-18)

This ADR documents regex pattern matching as the current enforcement implementation. Per ADR-010, enforcement is defined by its properties (deterministic, testable, predictable, explainable) rather than by this specific mechanism. Future enforcement may include non-pattern signals — ML confidence scores, structural analysis, behavioral patterns — provided they maintain the same properties. ML alternatives are deferred to the automated intelligence capability described in ADR-010. The human-curated learning loop (telemetry → feedback → curation → pattern improvement, per ADR-009) is a separate, operational capability that already extends enforcement effectiveness beyond static patterns through observation and human-directed improvement.
